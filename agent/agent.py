        ...
        while(True):
            for level in range(K):
                ...
                if e['clock']==e['trigger']:
                    ...
                    if level==K-1:
                        #print 'level',level,'start',start_reg_idx,'end',end_reg_idx
                        self.get_ipaddrs(self.read_reg_range(self.ptr_reg_name,start_reg_idx,end_reg_idx))
                e['clock'] += ALPHA
            sleep(ALPHA/1000.0)
    
    def get_ipaddrs(self,epochptrs):
        key=0
        for ptr in epochptrs:
            print 'pnt val:',ptr
            for x in range(32):
                if(ptr & (1<<x)):
                    print 'Found Key:',key,'IP:',self.key_ipaddrs[str(key)]
                key += 1
